 ...
 * 简 述：数据库执行类
 */
public class AndroidDBExecutor {
    ...
    public static synchronized <T extends DbModel> boolean insert(T t) {
        ...
        return DBManager.getDefaultDatabase().insert(table.getTableName(), null, contentValues) > 0;
    }
    ...
    public static synchronized <T extends DbModel> boolean insert(List<T> list) {
        ...
        SQLiteDatabase sqLiteDatabase = DBManager.getDefaultDatabase();
    ...
    public static synchronized SQLiteDatabase getSQLiteDatabase() {
        return DBManager.getDefaultDatabase();
    }
